...
export default async function PetsMiddleware(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (value) {
    let { name, gender, type, image, foundDate, foundLocal, contact, idUser } =
      value as Pet

    if (!name || !gender || !Object.values(PetType).includes(type) || !image) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: 'Invalid Pet data' })
    }

    if (!foundDate || !foundLocal) {
      return res
        .status(HttpStatus.BAD_REQUEST)
        .json({ error: 'Invalid Found data' })
    }
